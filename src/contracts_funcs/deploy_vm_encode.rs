...
pub fn encode_vmbyte_bool(b: bool) -> Result<Vec<u8>> {
    Ok(encode_vmbyte(VmValType::Bool, vec![b as u8]))
}

pub fn encode_vmbyte_i256(n: I256) -> Result<Vec<u8>> {
    let bytes = encode_i256(n);
    Ok(encode_vmbyte(VmValType::I256, bytes))
}

pub fn encode_vmbyte_u256(n: U256) -> Result<Vec<u8>> {
    let bytes = encode_u256(n);
    Ok(encode_vmbyte(VmValType::U256, bytes))
}
...
pub fn encode_vmbyte_vec(hex_bytes: &Vec<u8>) -> Result<Vec<u8>> {
    let size = encode_i32(hex_bytes.len() as i32);

    let final_bytes = [size, hex_bytes.clone()].concat();
    Ok(encode_vmbyte(VmValType::ByteVec, final_bytes))
}
