...
fn get_parents_from_source(source_info: &SourceInfo, index: usize) -> Vec<String> {
    let source_code = &source_info.source_code;
    let from_index = if source_info.kind == SourceKind::Interface {
        source_code[index..]
            .find(&source_info.name)
            .map(|pos| pos + source_info.name.len())
    } else {
...
fn fetch_dependencies(
    ...
    if cache.contains(&contract.name) {
        return Err(anyhow::anyhow!(
            "Circular dependency detected for contract: {}",
            contract.name
        ));
    }
    cache.insert(contract.name.clone());
    let mut all_parents = HashSet::new();

    if let Some(parents) = parents_per_contract.get(&contract.name) {
        for parent_id in parents {
            if let Some(parent) = source_infos.iter().find(|s| &s.name == parent_id) {
    ...
    dependencies.insert(contract.name.clone(), all_parents_vec.clone());
    Ok(())
}
...
fn get_dependencies(source_infos: &Vec<SourceInfo>) -> Result<HashMap<String, Vec<String>>> {
    ...
    for source in source_infos {
        if matches!(
            ...
            if !dependencies.contains_key(&source.name) {
                fetch_dependencies(
                    source_infos,
                    source,
                    &parents_per_contract,
...
impl Artifact {
    ...
    pub fn get_changed_sources(&self, source_infos: &Vec<SourceInfo>) -> Result<Vec<String>> {
        ...
        for source_info in source_infos {
            if let Some(info) = self.infos.get(&source_info.name) {
                if info.source_code_hash != source_info.source_code_hash {
                    result.insert(source_info.name.clone());
                    for (key, deps) in &dependencies {
                        if deps.contains(&source_info.name) {
        ...
        for name in self.infos.keys() {
            if !source_infos.iter().any(|s| &s.name == name) {
