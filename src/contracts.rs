...
impl ContractsSubcommands {
    pub async fn run(self, url: &str, config: &Config, network: &Network, network_id: NetworkType) -> Result<()> {
        let value: Value = match self {
            ...
            Self::Test { method_name, private_key, contract_id , args, exiting_contracts} => {
                let private_key = if let Some(path) = private_key {
                    let private_key: Box<dyn PrivateKey> =
                        Box::new(GLSecp256k1PrivateKey::new(&path)?);
                    Some(private_key)
